...
## Amendment 1 (owner, 2026-09-09): scroll choreography + hero interactivity + display font swap

### Font swap (whole page display role)
Replace Baloo 2 with **Fraunces** (Google Fonts; closest free kin of Mackinac's warm storybook display serif; use opsz axis + weights 500–900; `"SOFT" 0, "WONK" 1` on H1/H2 for the playful wonk where the API allows). DM Sans (body/UI), Caveat (human marks), JetBrains Mono (data) unchanged. Update every display-rule font-family and the direction-contract comment accordingly.

### #workflow — original fidelity + scroll-driven narrative (updated after live-site research 2026-09-09)
RESTORE original live behavior first: six pills clickable + Enter/Space emit a colored circle falling (rAF, ~0.8s fall) toward the terminal; on landing append `> <line>`; terminal keeps latest 6 lines; initial line `> session open · waiting for signals`; auto signals from random pills (~2.4s interval, first ~1.2s); clicked pill additive. Per-pill landed lines (verbatim from original): Note→`read the note · 3 paragraphs`; Comment→`comment folded into the plan`; Reaction→`reaction noted · priority up`; Reply→`reply answered in the thread`; Yes→`yes received · build starts`; Accept→`accepted · receipt written`. WATERLINE label rides a continuously redrawn dashed sine path. Replace our current static six-line terminal with this protocol.
THEN add the owner-requested scroll narrative on a sticky viewport-tall stage: scroll progress through the stage drives the sequence — Note highlights → its signal falls → line lands → next signal (Comment/Reaction) → … → Accept lands the receipt line. Clicks remain additive at any point. Below the stage the 4 ticket columns stay static verbatim. `prefers-reduced-motion` (match original): no falling/animation — pills neutral, terminal pre-seeded with lines 0,1,4 (`read the note · 3 paragraphs`, `comment folded into the plan`, `yes received · build starts`), figure pinned-off. Figure aria-label unchanged; no aria-live.

### Hero — original fidelity + interactive layered scene
RESTORE original figure behavior: auto-cycle stations Write→Plan→Build→Check→Watch→Write (1.8s hold, rAF morph of the boat along the route); pointerenter/focus pauses auto and selects that station; leave/blur resumes; selected station indicated by stroke-width 2 vs 1; stage visuals per original (list/grid group fades from Build; dashed connector through Plan onward; two sine waves at Check/Watch).
THEN add (owner liked Spade's `relative overflow-visible` energy): pointer-parallax on art layers (≤ 8px toward cursor, eased, `@media (pointer: fine)` + motion-safe); clicking a station also sails the paper-boat art toward it (≤600ms, instant under reduced motion); lighthouse beam slow 8s sweep; keep sparkle pulse. Hero stack `position: relative`, overflow-visible layers; clip only decorative bleed. Reduced-motion: figure initializes at Watch, no auto-cycle (match original).

All prior rules still apply (verbatim copy, palette, AA pairs, craft-floor bans, no entrances/scroll-jacking).
