...
- **§7.1 "Mode hierarchy"** — the Engineering/Planning control is no longer a standalone dominant switch rendered above the setup row and composer. It moves into the composer container's top row, right-aligned, with the Engineering-only setup pills left-aligned on the same row.
...
- **Header and intro copy** — the page's visible title is `New session` (with the subtitle `Start governed work with the right mode and context.`), and the centered intro heading is `What would you like to build?` with the body `Engineering sessions analyze, propose, and deliver software changes. You approve every proposal before work proceeds.`. The previous `Start Session` heading and its intro subtitle are removed.
- **Setup trigger copy** — the two Engineering-only triggers are icon pills whose initial placeholders are `Choose system / repositories` and `Choose component`; after selection they show the selected system name and the selected component name/count respectively.
...
- The New Session page intro (image, heading, body).
...
2. **Intro** (new, above the composer)
   ...
   - Visible heading `What would you like to build?` — a subordinate heading (`h2`) to the page's `h1`.
   - Body: `Engineering sessions analyze, propose, and deliver software changes. You approve every proposal before work proceeds.`

3. **Unified composer container** (exactly one composer container; no separate mode/setup regions outside it)
   - **Top row**
     - Left cluster: two Engineering-only icon pills — the `Choose system / repositories` pill and the `Choose component` pill. Their initial placeholders are the literal strings above; after selection the system pill shows the selected system name and the component pill shows the selected component name/count. The initial placeholders are specific to a fresh New Session and do not reflect the sidebar active-system context until a selection is made.
     - Right cluster: Session Mode radio group (Engineering / Planning segmented control) with its `Session mode` accessible label, right-aligned on the same row.
     - In Planning mode, both pills are removed from the DOM; the Session Mode radio group remains, right-aligned.
...
- **Intro:** centered; the illustration is compact (constrained height so the whole page fits 1200×720 — maximum image height ≈ 140px, width auto, no stretching); the `What would you like to build?` heading uses 20px (2XL) primary as the subordinate heading; the body uses 13px (MD) secondary/muted-AA.
...
- The intro heading `What would you like to build?` is a subordinate heading (`h2`) so the single-`h1` invariant holds.
...
- **System/repository** and **Component** pills keep their button semantics, accessible names, `aria-haspopup` (`dialog` / `menu`), and `aria-expanded`. Their initial accessible names are the placeholders `Choose system / repositories` and `Choose component` (reflecting the fresh-New-Session state, not the sidebar active system). In Planning they are removed from the accessibility tree (removed from the DOM), not merely visually hidden.
...
- Focus order follows visual/DOM order: header (non-interactive, skipped) → intro (non-interactive, skipped) → setup pills (Engineering) → Session Mode → textarea → input toolbar (`Attach file` → `Add text document` → Execution Profile → `Voice input` → `Send`/`Start planning`) → footer (`Reviews waiting`; disclaimer non-interactive).
...
2. **Engineering-only setup selectors:** `Choose system / repositories` opens the repository modal; `Choose component` opens the anchored component menu; Planning removes both selectors from the DOM. Initial placeholders are shown on a fresh New Session regardless of the sidebar active-system context; after selection the pills show the selected system name and the selected component name/count.
...
3. The page renders an intro above the composer containing `/assets/konteks/empty-sessions.png` (decorative, `alt=""`), the visible heading `What would you like to build?`, and the body `Engineering sessions analyze, propose, and deliver software changes. You approve every proposal before work proceeds.`.
...
6. After selection, the system pill shows the selected system name and the component pill shows the selected component name/count.
7. In Planning mode, both setup pills are absent from the DOM and the Session Mode radio group remains right-aligned in the top row.
...
14. The Engineering-only setup selectors preserve their existing overlay behavior (`Choose system / repositories` opens the repository modal; `Choose component` opens the anchored component menu).
...
- **Placeholders:** none. Asset path, header title/subtitle, approval indicator, intro heading/body, setup-pill placeholders, and disclaimer copy are exact and match the approved reference layout.
...
- **Ambiguity:** the disclaimer copy is character-exact (including the trailing period); the header left/right split, the intro heading/body, the setup-pill initial placeholders and post-selection labels, the nested input-box toolbar grouping (including Execution Profile), and the page-level footer grouping are each stated as a numbered acceptance criterion with a matching test strategy. The heading hierarchy is fixed (`New session` is the single `h1`; `What would you like to build?` is the subordinate `h2`).
