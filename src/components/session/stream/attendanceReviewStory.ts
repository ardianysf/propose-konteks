...
export const ATTENDANCE_REVIEW_STORY: StreamStoryEntry[] = [
  // 1 — User request bubble (message prose + two attachment cards).
  {
    ...
    data: {
      ...
      message:
        'Review the MyTok ↔ BSI HRIS attendance integration before Friday’s release. Monday’s payroll cut-off depends on check-in events from the MyTok mobile app landing in HRIS as attendance records: trace the sync path end to end, check timezone and overnight-shift handling, verify idempotency on retries, and report findings with evidence — no production fixes in this session.\n\nContext you will need: the sync worker queues events from the mobile check-in API (`canteen-api`) and replays them into the HRIS staging tables every five minutes. Last week’s migration `20260901` reshuffled the `attendance_records` schema, and the overnight canteen crew reported two shifts landing on the wrong day after the DST discussion — payroll wants certainty that the boundary fix from `PR #1301` actually holds before they lock the period.\n\nPlease also sanity-check the retry path: the worker is supposed to deduplicate on the mobile event id, and we have seen duplicate rows after connection drops during the August cut-off. If you need a bigger sample, the August export is attached.\n\nRun everything against staging — production stays read-only for this session.',
      chips: [
