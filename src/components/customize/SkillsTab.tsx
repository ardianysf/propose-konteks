/*
 * SkillsTab — the top-level Customize → Skills tab content (Task 9
 * Part C, spec §11, AC38).
 *
 * The preserved Skills functionality and content inside the new shell
...
export default function SkillsTab() {
  ...
  const items: PreservedContentItem[] = SKILLS_SECTION.items.map((item) => ({
    ...item,
  ...
  return (
    <section className="kx-customize-tab kx-customize-tab--skills">
      <header className="kx-customize-tab__bar">
        <h3 className="kx-customize-tab__title">{SKILLS_SECTION.title}</h3>
        <button
          type="button"
          className="kx-btn kx-btn--ghost kx-preserved__action"
        >
          {SKILLS_SECTION.actionLabel}
        </button>
      </header>

      <p className="kx-preserved__count">{preservedCountLine(items)}</p>

      <ul className="kx-preserved__list" aria-label={SKILLS_SECTION.listLabel}>
        {items.map((item) => (
      ...
      <p className="kx-preserved__note">{SKILLS_SECTION.note}</p>
    </section>
  )
}
